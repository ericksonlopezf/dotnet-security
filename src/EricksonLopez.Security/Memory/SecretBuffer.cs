...
public sealed class SecretBuffer : ISecretBuffer
{
    ...
    /// </summary>
    // Stryker disable once Block : Finalizer execution during GC
    ~SecretBuffer()
    {
        // Stryker disable once Statement,Boolean : Finalizer execution during GC
    ...
    public void Dispose()
    {
        // Stryker disable once Boolean : Dispose passes false for fromFinalizer
        DisposeCore(fromFinalizer: false);
        // Stryker disable once Statement : Finalizer suppression for IDisposable
    ...
    internal void DisposeCore(bool fromFinalizer)
    {
        ...
        // Stryker disable once Boolean,Logical : Finalizer guard preventing ArrayPool return during GC
        if (!fromFinalizer)
        {
