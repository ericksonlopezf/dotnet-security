...
public sealed class HmacSha256TokenHasher : ITokenHasher, IDisposable
{
    ...
    public void Dispose()
    {
        // Stryker disable once Block,Boolean,Statement : Idempotent dispose guard
        if (_disposed)
        {
            return;
        }
    ...
    public bool TryHashToken(ReadOnlySpan<char> token, Span<char> destination, out int charsWritten)
    {
        ...
        {
            // Stryker disable once Statement : Defense-in-depth wiping of ephemeral stack hash buffer
            CryptographicOperations.ZeroMemory(hash);
            // Stryker disable once Statement : Defense-in-depth wiping of ephemeral stack token buffer
