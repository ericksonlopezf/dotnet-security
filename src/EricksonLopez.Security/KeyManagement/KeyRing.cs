...
public sealed class KeyRing : IKeyRing, IEncryptionKeyProvider, IDisposable
{
    ...
    public KeyRing(IKeyStore keyStore, KeyRingOptions? options, IKeyRevocationNotifier? revocationNotifier)
    {
        ...
        if (revocationNotifier != null)
        {
            _revocationSubscription = revocationNotifier.Subscribe((keyId, version, purpose) =>
            {
                ...
                // Stryker disable once Statement : Defense-in-depth purpose eviction redundant with InvalidateKey active cache scan
    ...
    public Result<CryptographicKey> GetActiveKey(KeyPurpose purpose) =>
        GetActiveKeyAsync(purpose).AsTask().GetAwaiter().GetResult();
    ...
    public Result<CryptographicKey> GetKey(KeyIdentifier keyId, KeyVersion version) =>
        GetKeyAsync(keyId, version).AsTask().GetAwaiter().GetResult();
    ...
    internal void ClearCache()
    {
        lock (_cacheLock)
        {
            ...
            // Stryker disable once Block : Memory zeroing loop block
            {
                // Stryker disable once Statement : Zero out evicted key material in memory defense-in-depth
                CryptographicOperations.ZeroMemory(entry.Bytes);
            }
            // Stryker disable once Statement : Clear active key cache
    ...
    public void InvalidateKey(KeyIdentifier keyId, KeyVersion version)
    {
        ...
        lock (_cacheLock)
        {
            // Stryker disable once Block,Statement,Equality,Boolean : Zero out evicted key material in memory defense-in-depth
    ...
    public void Dispose()
    {
        // Stryker disable once Statement : Resource cleanup on Dispose
        _revocationSubscription?.Dispose();
        // Stryker disable once Statement : Memory scrubbing on Dispose
        ClearCache();
        // Dispose all semaphores to release any waiting threads.
        // Stryker disable once Block : Dispose cached semaphores
    ...
    public async ValueTask<Result<CryptographicKey>> GetActiveKeyAsync(KeyPurpose purpose, CancellationToken cancellationToken = default)
    {
        ...
        lock (_cacheLock)
        {
            // Stryker disable once Logical,Equality,Block : Fast path cache hit validation; default struct Expires is MinValue which is never > UtcNow
        ...
        try
        {
            ...
            lock (_cacheLock)
            {
                // Stryker disable once Logical,Equality : Double-checked cache hit validation; default struct Expires is MinValue which is never > UtcNow
                if (_activeKeyCache.TryGetValue(purpose, out var cached) && cached.Expires > DateTimeOffset.UtcNow)
                {
                    return new CryptographicKey(cached.Metadata, SecretBuffer.FromSpan(cached.Bytes));
                }
        ...
        {
            // Stryker disable once Statement : Lock release in finally block
            semaphore.Release();
        }
    }
    ...
    private async Task<Result<CryptographicKey>> FetchAndCacheActiveKeyAsync(KeyPurpose purpose, CancellationToken cancellationToken)
    {
        ...
        if (keyResult.IsSuccess && EffectiveCacheTtl > TimeSpan.Zero)
        {
            ...
            lock (_cacheLock)
            {
                // Stryker disable once Block,Statement,Equality,Boolean : Zero out superseded key material in memory defense-in-depth
    ...
    internal void PruneKeyCache(DateTimeOffset now)
    {
        foreach (var kvp in _keyCache)
        {
            // Stryker disable once Equality : Cache expiry boundary
            if (kvp.Value.Expires <= now)
            {
                // Stryker disable once Block,Statement,Equality,Boolean : Zero out pruned key material in memory defense-in-depth
        ...
        if (_keyCache.Count >= MaxCacheCapacity)
        {
            ...
            // Stryker disable once Equality : Non-negative overflow check
            if (overflowCount > 0)
            {
    ...
    public async ValueTask<Result<CryptographicKey>> GetKeyAsync(KeyIdentifier keyId, KeyVersion version, CancellationToken cancellationToken = default)
    {
        ...
        lock (_cacheLock)
        {
            // Stryker disable once Logical,Equality : Fast path cache hit validation; default struct Expires is MinValue which is never > UtcNow
            if (_keyCache.TryGetValue(cacheKey, out var cached) && cached.Expires > DateTimeOffset.UtcNow)
            {
                // Stryker disable once Block,String,Equality : Defensive check for revoked state in cached entries
