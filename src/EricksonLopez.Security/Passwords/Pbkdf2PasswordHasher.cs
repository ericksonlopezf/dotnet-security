...
public sealed class Pbkdf2PasswordHasher : IPasswordHasher
{
    ...
    /// <summary>
    /// Maximum iteration count accepted from a stored hash string during verification (DoS prevention, FINDING-NEW-01).
    /// </summary>
    public const int MaxStoredIterations = 600_000;
    ...
    public string HashPassword(ReadOnlySpan<char> password)
    {
        ...
        // Stryker disable once Statement : Wiping ephemeral stackalloc buffers in finally/return
        CryptographicOperations.ZeroMemory(derivedKey);

        // Stryker disable once Statement : Stopwatch stop
        sw.Stop();
    ...
    public PasswordVerificationResult VerifyPassword(ReadOnlySpan<char> password, string hashedPassword)
    {
        ...
        var resultTag = result switch
        {
            ...
            // Stryker disable once String : Telemetry tag value
    ...
    private PasswordVerificationResult VerifyPasswordCore(ReadOnlySpan<char> password, string hashedPassword)
    {
        ...
        // Stryker disable once Equality,Logical : Iteration count defense-in-depth bounds
        ...
        // Stryker disable once Equality,Logical : Pre-allocation stackalloc defense-in-depth bounds validated by exact byte bounds on line 228
        ...
        // Stryker disable once Equality,Logical : Decoded byte length defense bounds
    ...
    private PasswordVerificationResult VerifyTier1Pbkdf2Format(ReadOnlySpan<char> password, string hashedPassword)
    {
        ...
        // Stryker disable once Equality,Logical : Cross-tier iteration count bounds
        if (storedIterations <= 0 || storedIterations > MaxStoredIterations)
        {
            return PasswordVerificationResult.Failed;
        }
        ...
        // FINDING-NEW-02 / SEC-MEM-1: Pre-allocation validation
        // Stryker disable once Equality,Logical : Pre-allocation stackalloc defense-in-depth bounds validated by exact byte bounds on line 291
