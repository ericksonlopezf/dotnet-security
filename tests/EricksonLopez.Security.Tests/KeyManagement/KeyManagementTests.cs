...
public sealed class KeyManagementTests
{
    ...
    {
        var store = new CountingKeyStore();
        var ring = new KeyRing(store, new KeyRingOptions { CacheTtl = TimeSpan.FromMinutes(10) }) { MaxCacheCapacity = 5 };
        ...
        ring.KeyCacheCount.Should().Be(5);
        store.GetKeyCount.Should().Be(5);

        // 6th key triggers capacity pruning down to 80% (int(5 * 0.8) = 4 keys remain, then 5th added -> 5 keys)
        var id5 = new KeyIdentifier("k_5");
        var meta5 = new KeyMetadata(id5, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES-256-GCM", DateTimeOffset.UtcNow);
        using var buf5 = SecretBuffer.CreateRandom(32);
        await store.SaveKeyAsync(new CryptographicKey(meta5, buf5));
        var res5 = await ring.GetKeyAsync(id5, KeyVersion.Initial);
        res5.IsSuccess.Should().BeTrue();
        res5.Value.Dispose();

        ring.KeyCacheCount.Should().Be(5);
        // k_4 was retained in cache (newest): fetching k_4 must NOT hit store
        var reget4 = await ring.GetKeyAsync(new KeyIdentifier("k_4"), KeyVersion.Initial);
        reget4.IsSuccess.Should().BeTrue();
        reget4.Value.Dispose();
        store.GetKeyCount.Should().Be(6, "k_4 was retained in cache so fetching it must not hit store");

        // k_0 was evicted as the oldest key when k_5 was added: fetching k_0 must hit the store again
        var reget0 = await ring.GetKeyAsync(new KeyIdentifier("k_0"), KeyVersion.Initial);
        reget0.IsSuccess.Should().BeTrue();
        reget0.Value.Dispose();
        store.GetKeyCount.Should().Be(7, "k_0 was evicted so fetching it again must hit store");

        // Re-adding k_0 evicted k_1 (next oldest): fetching k_1 must now hit store
        var reget1 = await ring.GetKeyAsync(new KeyIdentifier("k_1"), KeyVersion.Initial);
        reget1.IsSuccess.Should().BeTrue();
        reget1.Value.Dispose();
        store.GetKeyCount.Should().Be(8, "k_1 was evicted when k_0 was re-added so fetching it must hit store");
    }

    [Fact]
    public async Task KeyRing_PruneKeyCache_RemovesExpiredKeys()
    {
        var store = new InMemoryKeyStore();
        var id = new KeyIdentifier("exp_key");
        var meta = new KeyMetadata(id, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES-256-GCM", DateTimeOffset.UtcNow);
        using var buf = SecretBuffer.CreateRandom(32);
        await store.SaveKeyAsync(new CryptographicKey(meta, buf));

        var ring = new KeyRing(store, new KeyRingOptions { CacheTtl = TimeSpan.FromMinutes(1) });
        var res = await ring.GetKeyAsync(id, KeyVersion.Initial);
        res.IsSuccess.Should().BeTrue();
        res.Value.Dispose();
        ring.KeyCacheCount.Should().Be(1);

        // Calling PruneKeyCache with past time does NOT remove it
        ring.PruneKeyCache(DateTimeOffset.UtcNow.AddMinutes(-5));
        ring.KeyCacheCount.Should().Be(1);

        // Calling PruneKeyCache with future time removes the expired key
        ring.PruneKeyCache(DateTimeOffset.UtcNow.AddMinutes(5));
        ring.KeyCacheCount.Should().Be(0);
    }

    [Fact]
    public async Task KeyRing_ZeroCacheTtl_DisablesCaching()
    {
        var store = new CountingKeyStore();
        var id = new KeyIdentifier("nocache_key");
        var meta = new KeyMetadata(id, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES-256-GCM", DateTimeOffset.UtcNow);
        using var buf = SecretBuffer.CreateRandom(32);
        await store.SaveKeyAsync(new CryptographicKey(meta, buf));

        var ring = new KeyRing(store, new KeyRingOptions { CacheTtl = TimeSpan.Zero });
        var res1 = await ring.GetKeyAsync(id, KeyVersion.Initial);
    ...
    [Fact]
    public async Task KeyRing_ExpiredKey_ReplacedInCache_OverwritesOldEntry()
    {
        var store = new InMemoryKeyStore();
        var id = new KeyIdentifier("replace_key");
        var meta = new KeyMetadata(id, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES-256-GCM", DateTimeOffset.UtcNow);
        using var buf = SecretBuffer.CreateRandom(32);
        await store.SaveKeyAsync(new CryptographicKey(meta, buf));

        // Use a 10ms TTL
        var ring = new KeyRing(store, new KeyRingOptions { CacheTtl = TimeSpan.FromMilliseconds(10) });
        var res1 = await ring.GetActiveKeyAsync(KeyPurpose.Encryption);
        res1.IsSuccess.Should().BeTrue();
        res1.Value.Dispose();
        ...
        // Second call: expired entry is in cache so TryGetValue in store-fetch path is true, zeroing old entry and replacing
        var res2 = await ring.GetActiveKeyAsync(KeyPurpose.Encryption);
    ...
    [Fact]
    public async Task InMemoryKeyStore_CapacityExceeded_ReturnsDetailedDescription()
    {
        var store = new InMemoryKeyStore { MaxKeysCapacity = 1 };
        var id1 = KeyIdentifier.New();
        var meta1 = new KeyMetadata(id1, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES", DateTimeOffset.UtcNow);
        using var buf1 = SecretBuffer.CreateRandom(32);
        var res1 = await store.SaveKeyAsync(new CryptographicKey(meta1, buf1));
        res1.IsSuccess.Should().BeTrue();

        var id2 = KeyIdentifier.New();
        var meta2 = new KeyMetadata(id2, KeyVersion.Initial, KeyPurpose.Encryption, KeyStatus.Active, "AES", DateTimeOffset.UtcNow);
        using var buf2 = SecretBuffer.CreateRandom(32);
        var res2 = await store.SaveKeyAsync(new CryptographicKey(meta2, buf2));
        res2.IsFailure.Should().BeTrue();
        res2.Error.Code.Should().Be("Security.StoreCapacityExceeded");
        res2.Error.Description.Should().Contain("InMemoryKeyStore capacity exceeded (1 keys).");
        res2.Error.Description.Should().Contain("This store is intended for testing and development only.");
        res2.Error.Description.Should().Contain("For production use, configure a persistent cloud key store.");
    }
