...
public sealed class TokenSecurityTests
{
    ...
    public void OpaqueTokenGenerator_GenerateToken_ProducesValidTokens()
    {
        ...
        var num16 = generator.GenerateNumericCode(16);
        Assert.Equal(16, num16.Length);
        Assert.All(num16, c => Assert.True(char.IsDigit(c)));
        Assert.Throws<ArgumentOutOfRangeException>(() => generator.GenerateNumericCode(17));
        Assert.Throws<ArgumentOutOfRangeException>(() => generator.GenerateNumericCode(0));
    }
    ...
    public void HmacSha256TokenHasher_Dispose_ScrubsPepperKey_AndThrowsOnSubsequentUse()
    {
        ...
        Assert.Throws<ObjectDisposedException>(() => hasher.HashToken(""));
        Assert.Throws<ObjectDisposedException>(() => hasher.VerifyToken("sample-token", hash));
        Assert.Throws<ObjectDisposedException>(() => hasher.VerifyToken("", ""));

        // Fresh hasher validates empty token with exact message
        var freshHasher = new HmacSha256TokenHasher();
        var exEmpty = Assert.Throws<ArgumentException>(() => freshHasher.HashToken(""));
        Assert.Contains("Token cannot be empty.", exEmpty.Message);
